module Middleman
  module Deploy
    module Methods
      class Rsync < Base
        attr_reader :clean, :flags, :host, :path, :port, :user, :password
        ...
        def process
          ...
          command   = "rsync #{flags} '-e sshpass -p #{self.password}  ssh -p #{self.port}' #{self.server_instance.build_dir}/ #{dest_url}"
